...
import { PrismaClientKnownRequestError, PrismaClientUnknownRequestError } from '@prisma/client/runtime/library.js'
...
export default class UserRepository {
	...
	async create(fullName, email, passwordHash, refreshToken, role, approverStatus, bankName, ifsc, bankAccNum, pan) {
		try {
			...
			if (err instanceof PrismaClientKnownRequestError) {
				...
				case 'P2002':
					throw new RepositoryError(`User ${err.meta.target} already exists`, DB_ERROR_CODES.UNIQUE_CONSTRAINT_VIOLATION, err.meta)
				case 'P2003':
					logger.error(err.message, { errorMetadata: err.meta.target })
	...
	async fetchAll() {
		try {
			...
			if (err instanceof PrismaClientKnownRequestError) {
				...
				case 'P2015':
					logger.error(err.message, { errorMetadata: err.meta.target })
					throw new RepositoryError(err.message, DB_ERROR_CODES.NO_RELATED_RECORDS, err.meta, err.stack)
				case 'P2017':
					logger.error(err.message, { errorMetadata: err.meta.target })
					throw new RepositoryError(err.message, DB_ERROR_CODES.RELATIONS_NOT_CONNECTED, err.meta, err.stack)
				case 'P2018':
					logger.error(err.message, { errorMetadata: err.meta.target })
					throw new RepositoryError(err.message, DB_ERROR_CODES.CONNECTED_RECORDS_NOT_FOUND, err.meta, err.stack)
				}
			} else if (err instanceof PrismaClientUnknownRequestError) {
				logger.error(err.message, { errorMetadata: err.meta })
				throw new RepositoryError(err.message, DB_ERROR_CODES.UNKNOWN_ORM_ERROR, err.meta, err.stack)
	...
	async fetchByEmail(email) {
		try {
			...
			if (err instanceof PrismaClientKnownRequestError) {
				switch (err.code) {
				case 'P2025':
					throw new RepositoryError(`User with ${err.meta.target}=${email} does not exist`, DB_ERROR_CODES.RECORD_DOES_NOT_EXIST, err.meta)
				}
			} else if (err instanceof PrismaClientUnknownRequestError) {
				logger.error(err.message, { errorMetadata: err.meta })
				throw new RepositoryError(err.message, DB_ERROR_CODES.UNKNOWN_ORM_ERROR, err.meta, err.stack)
